...
- Cross-process participation is off by default (`irc.crossProcess`); enabling it makes an interactive, `rpc-ui`, or ACP session attach to (and, on demand, start) the project's broker at launch. A headless (`-p`/RPC) run attaches only when a broker is already listening for the project, and never reconnects if that connection drops — so a scripted loop of one-shot invocations never leaves broker processes behind.
...
- Cross-process: `list`'s `limit` applies to the merged local+remote roster, with `details.counts.remote` counting every cross-process peer in the roster (not just the rows `limit` let through) and each remote row flagged `remote: true`; direct sends to a remote peer ack within 10 s, broadcast legs to remote peers within 2 s; a remote `await` peer never observed running aborts after a bounded ~10 s grace window; the `irc.sync`/`irc.ack`/`irc.detach` RPCs time out after 5 s, and a wire `irc.send` timeout is clamped to 1 ms–60 s.
