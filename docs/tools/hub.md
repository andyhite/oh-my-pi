...
Agents in other `omp` processes attached to the same canonical project/working-directory broker appear in `hub list` as `<peer-name>/<agent-id>` and can be messaged directly, exactly like a local subagent id. Peer names are matched exactly (`Alpha` and `alpha` are distinct instances). A bare id present in the local roster always resolves locally; a bare id that two or more *remote* peers advertise is rejected with an `Ambiguous peer "<id>" — N omp processes advertise it: ...` error naming every qualified candidate.

- Set this process's peer name with `--name <name>` at startup or `/peer <name>` at runtime; both sanitize to `[A-Za-z0-9_-]`, 1-48 chars and report when the value they applied differs from what was typed — neither rejects a non-canonical value outright. A name already held by another process in the same project scope is granted a numeric suffix (`Alpha-2`); that suffixed grant is pinned as this process's address until an explicit `--name`/`/peer` changes it, rather than silently reclaiming the unsuffixed name once the holder detaches.
- Cross-process participation is off by default (`irc.crossProcess`); enabling it makes an interactive, `rpc-ui`, or ACP session attach to (and, on demand, start) the project's broker at launch. A headless (`-p`/RPC) run attaches only when a broker is already listening for the project, and never spawns a broker or retries in the background — if the connection drops it re-attaches only when a later `hub` messaging op finds a broker already listening, so a scripted loop of one-shot invocations never leaves broker processes behind.
...
- Cross-process messages to or from your subagents surface as relay cards on the main session, exactly like local agent↔agent traffic.
- A `send await:true` waiter matches the peer's fully qualified id at send time. If that process renames itself with `/peer` before it answers, the reply arrives under the new name, misses the waiter, and lands in your inbox — the awaiting send then reports no reply. Re-read `inbox` after a peer rename.
...
- Cross-process: `list`'s `limit` applies to the merged local+remote roster, with `details.counts.remote` counting every cross-process peer in the roster (not just the rows `limit` let through) and each remote row flagged `remote: true`; direct sends to a remote peer ack within 10 s, broadcast legs to remote peers within 2 s; a remote `await` peer never observed running aborts after a bounded ~10 s grace window; the `irc.sync`/`irc.ack`/`irc.detach` RPCs time out after 5 s, and a wire `irc.send` timeout is clamped to 1 ms–60 s; a cross-process message body is capped at 256 KiB (local delivery is uncapped).
