...
- Cross-process: `list`'s `limit` applies to the merged local+remote roster, with `details.counts.remote` counting every cross-process peer in the roster (not just the rows `limit` let through) and each remote row flagged `remote: true`; direct sends to a remote peer ack within 10 s, broadcast legs to remote peers within 2 s; a remote `await` peer never observed running aborts after a bounded ~10 s grace window.
