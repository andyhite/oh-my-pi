...
- Process identity system for cross-process messaging: immutable UUID tokens per agent, dual-track name state (requested vs. broker-granted) with collision resolution, and sanitized display names
- Cross-process attachment status tracking via `crossProcessIrcStatus()` reporting whether the local process is registered in its scope's IRC registry, with error details when unavailable
